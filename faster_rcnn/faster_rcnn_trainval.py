...
#big_dir = 2d_bigdataset_new_{ax_fix}_r3'
phase = 'Train'
num_epochs_no_roi = args.epochs_frozen
num_epochs_full = args.epochs_full
model_folder = f'faster_rcnn_50_{ax_fix}_correctbigset_3classes_e{num_epochs_full}_o{args.overfit}'
if num_epochs_no_roi != num_epochs_full:
    model_folder = model_folder + f'_noroi{num_epochs_no_roi}'
...
print('big dir i.e. data: ', big_dir)
...
def main():
    ...
    np.save(f'{model_dir}/train_loss_frozen.npy', train_losses)
    ...
    if args.overfit:
        model.eval()
        print('\n\nAttempting a prediction for the train image')
        # run predictions on the training image and see what is returned
        with torch.no_grad():
            for images, targets in train_loader:
                images = list(image.to(device) for image in images)
                ...
                for prediction in predictions:
                    ...
                    #output_file = os.path.join(output_dir, f'printtest_{label_name}')
