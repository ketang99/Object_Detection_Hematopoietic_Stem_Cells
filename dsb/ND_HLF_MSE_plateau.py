...
mname = f'test_mse_plateauscheduler_relu_epoch{epochs}_defanch_sgd_red_dsb__pneq_2patch__e-1'
...
def train_nodulenet(data_loader, net, loss, epoch, loss_array, optimizer, args, epoch_save, model_filename, save_path, loss_path):
   ...
   # for (i, batch) in enumerate(data_loader):
    ...
    for (i, batch) in enumerate(data_loader):
        ...
        loss_array.append(loss_output.item())
...
def validate_nodulenet(data_loader, net, loss, epoch, loss_array, args, save_path, loss_path):
    ...
    for (i, batch) in enumerate(data_loader):
        ...
        # loss_output = loss_output[0].item()
        raw_metrics.append(loss_output)    
    end_time = time.time()
   ...
   # return
    ...
    for i in range(epochs):
       ...
       # lrates.append(optimizer.param_groups[0]['lr'])
        ...
        print('\n***\n')
        print('Validation:')
        val_epoch_loss = validate_nodulenet(val_loader_nod, nod_net, loss, i, val_losses, args, model_save_path, loss_save_path)
    ...
    np.save(f'{loss_save_path}/Train_loss_metrics_{epochs}.npy', train_losses)
    np.save(f'{loss_save_path}/Val_loss_metrics_{epochs}.npy', val_losses)
