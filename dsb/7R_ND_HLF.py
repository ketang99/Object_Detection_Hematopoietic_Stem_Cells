...
train_filename = '12A+6W_ev_Region+1_Merged+filtered+ANNOTATED+CROP.ims'
mname = f'test_inplacefalse_relu_defanch_sgd_red_dsb_epoch{epochs}_pneq_2patch__e-1'
...
'''
Add the loss for 7 regressors here. This script will be cls (BCE) and reg (smooth L1)
add the print statements for troubleshooting
'''
...
class Loss(nn.Module):
    ...
    def forward(self, output, labels, train=True):
        ...
        if self.num_hard > 0 and train:
            neg_output, neg_labels, hard_idcs = hard_mining(neg_output, neg_labels, neg_idcs, nhard)
        ...
        if len(pos_output) > 0:
            ...
            pz, ph, pw, prz, prh, prw = pos_output[:, 1], pos_output[:, 2], pos_output[:, 3], pos_output[:, 4], pos_output[:, 5], pos_output[:, 6]
            lz, lh, lw, lrz, lrh, lrw = pos_labels[:, 1], pos_labels[:, 2], pos_labels[:, 3], pos_labels[:, 4], pos_labels[:, 5], pos_labels[:, 6]
            ...
            print('neg_prob: ', neg_prob, neg_prob.size(), neg_prob.device)
            ...
            classify_loss = 0.5 * self.classify_loss(pos_prob, pos_labels[:, 0]) + 0.5 * self.classify_loss(
                neg_prob, neg_labels[:, 0] + 1)
            pos_correct = (pos_prob.data >= 0.5).sum()
            pos_total = len(pos_prob)
        else:
            regress_losses = [0, 0, 0, 0, 0, 0]
            classify_loss = 0.5 * self.classify_loss(neg_output[:, 0], neg_labels[:, 0] + 1)
            pos_correct = 0
            pos_total = 0
        ...
        return [loss, classify_loss.item()] + regress_losses_data + [pos_correct, pos_total, neg_correct, neg_total]
...
def main():
   ...
   # nod_net = torch.nn.DataParallel(nod_net)
    ...
    config['anchors'] = [max_bb*f for f in [0.25, 0.5, 0.75, 1.0]]
    ...
    train_dataset = dsb.HLFBoneMarrowCells(train_filename, ims_dir, config, phase='Train')
    val_dataset = dsb.HLFBoneMarrowCells(train_filename, ims_dir, config, phase='Val')
    ...
    for name, param in nod_net.named_parameters():
        print(f"Parameter name: {name}")
        # print(f"Parameter value: {param.data}")
        print(f"Parameter shape: {param.shape}")
        print(f"Parameter mean: {param.data.mean()}")
        print(f"Parameter norm: {param.data.norm()}")
        print()
